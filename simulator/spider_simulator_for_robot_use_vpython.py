__author__ = 'chernobrovyivs'

from vpython import *

windowtitle = 'SpiderBot Simulator'
newscene = canvas(width=1920,height=1080,title=windowtitle)
...
def create_legs(i):
    ...
    coxa_frame = cylinder(pos=vec(0, 0, 0), length=coxa_len, axis=vec(1, 0, 0), radius=6, color=color.red)

    femur_frame = cylinder(pos=vec(coxa_len, 0, 0), length=femur_len, radius=6, color=color.green,  axis=vec(1, 0, 0))

    tibia_frame = cylinder(pos=vec(femur_len, 0, 0), length=tibia_len, radius=6, color=color.blue, axis=vec(0, 0, -1))
...
def draw_legs(leg, a, b, g):
    ...
    femur[leg].axis = vec(x_dir * cos(a), y_dir * sin(g), sin(a))  #femur[leg].axis = vec(cos(a), 0, sin(a)) --- vec(x, y, z)
    femur[leg].length=femur_len

    tibia[leg].axis = vec(x_dir * cos(b), 0, -sin(b))  #tibia[leg].axis = vec(-cos(b), 0, -sin(b))
